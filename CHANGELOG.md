...
### Added
- Content expansion (`docs/content/`): added eight new achievements covering gardening, care depth, and longevity (`garden-first-bloom`, `garden-tender`, `feed-the-favorite`, `hundred-soft-touches`, `pen-pal`, `the-stack-on-the-desk`, `clean-rescue`, `caught-napping`), bringing the authored set from twelve to twenty. All twelve original entries also gained a short `Bubble` line (<=80 chars) for the chat surface, alongside the existing longer `Flavor` reference text.
- Content expansion (`docs/content/letter-beats.md`): added a new section 5 catalogue of concrete letter beats — three mood (sad / excited / anxious), three topic (question-about-pet / question-about-day / asks-for-advice with sensitive-case split), three oddball (gibberish / single-word / angry) — each with scenario summary, voice direction, and two-to-three example reply lines.
- Voice guide (`docs/content/voice-guide.md`): added a "no volume-matching" posture rule under section 5 to protect the pal's register when input is loud (ALL CAPS, exuberant, angry).
